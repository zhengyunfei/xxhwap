...
public class SchoolCardControll {
     ...
     * @return
     */
	@RequestMapping(value = "/card/cancelSale.html", method = RequestMethod.POST)
	@ResponseBody
	public Map<String,Object> cancelSale(HttpServletRequest request,
		...
		try{
			...
			TudouBookInfo book=bookService.findById(id);
				...
				if(count==0){
					flg=bookService.delBookById(id);//step1 这届删除
				}else{
					//更改有效标示
	 ...
	 * @return
	 */
	@RequestMapping(value = "/card/dealFail.html", method = RequestMethod.POST)
	@ResponseBody
